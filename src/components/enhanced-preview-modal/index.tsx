...
const getEnhancededitedResume = async (url: string, { arg }: { arg: string }) => {
  ...
  return (
    <Dialog open={true} >
      <DialogContent showCloseButton={false} className="overflow-auto min-w-4xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="text-3xl mx-auto">Resume Preview</DialogTitle>
        </DialogHeader>
        {
...
const EditResume = ({ editedResume, setEditedResume }: { editedResume: Resume, setEditedResume: any }) => {
  const [activeSection, setActiveSection] = useState("Personal");

  return (
    <div>
      <Tabs value={activeSection} onValueChange={(val) => setActiveSection(val)} className="mb-8">
